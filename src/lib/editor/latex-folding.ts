 ...
 */
export const latexFoldService = foldService.of((state: EditorState, lineStart: number, lineEnd: number) => {
  ...
  // Fold sections
  const sectionMatch = lineText.match(/\\(chapter|section|subsection|subsubsection|paragraph|subparagraph)\{/);
  if (sectionMatch) {
    ...
    for (let i = line.number + 1; i <= state.doc.lines; i++) {
      const searchLine = state.doc.line(i);
      const nextSectionMatch = searchLine.text.match(/\\(chapter|section|subsection|subsubsection|paragraph|subparagraph)\{/);
  ...
  if (openBrace !== -1 && !lineText.includes('}', openBrace)) {
    ...
    for (let i = line.number + 1; i <= state.doc.lines && braceCount > 0; i++) {
      ...
      if (braceCount === 0) {
        if (i > line.number + 1) { // Only fold if it spans multiple lines
          return {
            from: line.to,
